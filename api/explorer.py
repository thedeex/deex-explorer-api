...
def get_header():
    response = deex_ws_client.request('database', 'get_dynamic_global_properties', [])
    return _add_global_informations(response, deex_ws_client)


@cache.memoize()
def get_account(account_id):
    return deex_ws_client.request('database', 'get_accounts', [[account_id]])[0]
...
def _get_account_id(account_name):
    if not _is_object(account_name):
        account = deex_ws_client.request('database', 'lookup_account_names', [[account_name], 0])
        return account[0]['id']
    else:
        return account_name
...
def _add_global_informations(response, ws_client):
    ...
    response["deex_market_cap"] = int(market_cap/100000000)

    if config.TESTNET != 1: # Todo: had to do something else for the testnet
        deexBtcVolume = ws_client.request('database', 'get_24_volume', ["DEEX", "BTC"])
        response["quote_volume"] = deexBtcVolume["quote_volume"]
    else:
...
def get_operation(operation_id):
    ...
    operation = _enrich_operation(operation, deex_ws_client)
    return operation
...
def get_full_account(account_id):
    account = deex_ws_client.request('database', 'get_full_accounts', [[account_id], 0])[0][1]
    return account
...
def get_assets():
    ...
    markets = deex_es_client.get_markets('now-1d', 'now', quote=config.CORE_ASSET_ID)
    deex_volume = 0.0 # DEEX volume is the sum of all the others.
    for asset_id in itertools.chain(markets.keys(), [config.CORE_ASSET_ID]):
        ...
        deex_volume += float(asset['volume'])
        results.append({
            'asset_name': asset['symbol'], # asset name
            'asset_id': asset_id, # asset id
            'latest_price': asset['latest_price'], # price in deex
            '24h_volume': float(asset['volume']) if asset_id != config.CORE_ASSET_ID else deex_volume, # 24h volume
...
def get_fees():
    return deex_ws_client.get_global_properties()


@cache.memoize()
def _get_asset_id_and_precision(asset_name):
    asset = deex_ws_client.request('database', 'lookup_asset_symbols', [[asset_name], 0])[0]
...
def get_asset(asset_id):
    ...
    if not _is_object(asset_id):
        asset = deex_ws_client.request('database', 'lookup_asset_symbols', [[asset_id], 0])[0]
    else:
        asset = deex_ws_client.request('database', 'get_assets', [[asset_id], 0])[0]
    
    dynamic_asset_data = deex_ws_client.get_object(asset["dynamic_asset_data_id"])
...
def get_object(object):
    return deex_ws_client.get_object(object)
...
def get_asset_holders_count(asset_id):
    ...
    return deex_ws_client.request('asset', 'get_asset_holders_count', [asset_id])


def get_asset_holders(asset_id, start=0, limit=20):
    asset_id = _ensure_asset_id(asset_id)
    asset_holders = deex_ws_client.request('asset', 'get_asset_holders', [asset_id, start, limit])
    return asset_holders


def get_workers():
    workers_count = deex_ws_client.request('database', 'get_worker_count', [])
    workers = deex_ws_client.request('database', 'get_objects', [ [ '1.14.{}'.format(i) for i in range(0, workers_count) ] ])
...
def _get_markets(asset_id):
    markets = deex_es_client.get_markets('now-1d', 'now', quote=asset_id)
...
def get_most_active_markets():
    markets = deex_es_client.get_markets('now-1d', 'now')
...
def get_order_book(base, quote, limit=False):
    ...
    order_book = deex_ws_client.request('database', 'get_order_book', [base, quote, limit])
    return order_book


def get_margin_positions(account_id):
    margin_positions = deex_ws_client.request('database', 'get_margin_positions', [account_id])
    return margin_positions


def get_witnesses():
    witnesses_count = deex_ws_client.request('database', 'get_witness_count', [])
    witnesses = deex_ws_client.request('database', 'get_objects', [ ['1.6.{}'.format(w) for w in range(0, witnesses_count)] ])
    result = []
...
def get_committee_members():
    ...
    committee_members = deex_ws_client.request('database', 'get_objects', [ ['1.5.{}'.format(i) for i in range(0, committee_count)] ])
...
def get_market_chart_data(base, quote):
    ...
    market_history = deex_ws_client.request('history', 'get_market_history', [base_id, quote_id, 86400, ago.strftime("%Y-%m-%dT%H:%M:%S"), now.strftime("%Y-%m-%dT%H:%M:%S")])
    ...
    for holder in holders:
        if 'follower_count' in holder:
            ...
            proxies.append({
                ...
                'deex_weight': proxy_amount,
                'followers': holder['follower_count'],
                'deex_weight_percentage': proxy_total_percentage
            })

    proxies = sorted(proxies, key=lambda k: -k['deex_weight']) # Reverse amount order
...
def _get_accounts_by_chunks_via_es(account_ids, chunk_size=1000):
    ...
    for i in xrange(0, len(account_ids), chunk_size):
        accounts = deex_es_client.get_accounts(account_ids[i:i+chunk_size], size=chunk_size)
...
def _get_accounts_by_chunks_via_ws(account_ids, chunk_size=1000):
    all_accounts = []
    for i in xrange(0, len(account_ids), chunk_size):
        accounts = deex_ws_client.request('database', 'get_accounts', [ account_ids[i:i+chunk_size] ])
        all_accounts.extend(accounts)
    return all_accounts

# FIXME: Should not be needed anymore when https://github.com/deex/deex-core/issues/1652 will be resolved.
...
@cache.memoize()
def _get_holders():
    balances = deex_es_client.get_balances(asset_id=config.CORE_ASSET_ID)
...
def get_witnesses_votes():
    ...
    proxies = proxies[:10]
    proxies = deex_ws_client.request('database', 'get_objects', [[ p['id'] for p in proxies ]])
...
def lookup_accounts(start):
    accounts = deex_ws_client.request('database', 'lookup_accounts', [start, 1000])
    return accounts


def lookup_assets(start):
    asset_names = deex_es_client.get_asset_names(start)
    return [ [ asset_name ] for asset_name in asset_names ]


def get_last_block_number():
    dynamic_global_properties = deex_ws_client.request('database', 'get_dynamic_global_properties', [])
...
def get_fill_order_history(base, quote):
    fill_order_history = deex_ws_client.request('history', 'get_fill_order_history', [base, quote, 100])
...
def get_daily_volume_dex_data():
    daily_volumes = deex_es_client.get_daily_volume('now-60d', 'now')
...
def get_referrer_count(account_id):
    ...
    count, _ = deex_es_client.get_accounts_with_referrer(account_id, size=0)
...
def get_all_referrers(account_id, page=0):
    ...
    _, accounts = deex_es_client.get_accounts_with_referrer(account_id, size=page_size, from_=offset)
...
def get_grouped_limit_orders(quote, base, group=10, limit=False):
    ...
    grouped_limit_orders = deex_ws_client.request('orders', 'get_grouped_limit_orders', [base, quote, group, None, limit])
