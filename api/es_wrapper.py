...
from services.deex_elasticsearch_client import es
...
def is_alive():
    ...
    s = Search(using=es, index="deex-" + find_string)
...
def get_trx(trx, from_=0, size=10):
    s = Search(using=es, index="deex-*", extra={"size": size, "from": from_})
